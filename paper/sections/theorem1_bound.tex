...
% (used throughout P1-07/P1-08). That additive form was checked with
...
Fix a design $S_{\mathrm{fit}}$ with total compute $C$, and suppose each arm's
estimate $\hat\mu_k(s^\star) = g(\hat\theta_k, s^\star)$ is obtained by (weighted)
least squares within the family (Assumption~\ref{asn:family}) on noisy observations
(Assumption~\ref{asn:noise}) of an arm whose true trend is
$\mu_k(s) = g(\theta_k, s) + h_k(s)$ with $h_k \in \mathcal{H}$
(Assumption~\ref{asn:misspec}). Let $v_k(C) = J(\theta_k^{\dagger}, s^\star)^\top
\Sigma_{\theta_k}(C) \, J(\theta_k^{\dagger}, s^\star)$ be the delta-method estimation
variance of $\hat\mu_k(s^\star)$ (Section~\ref{sec:theorem1-step2} below), and
$\sigma^2_{\mathrm{extrap}, k} = \sigma^2_{\mathrm{extrap}, k}(S_{\mathrm{fit}})$ as in
Definition~\ref{def:sigma2-extrap}. Then
\[
  \mathbb{P}[\hat{k} \ne k^\star]
  \;\le\; \sum_{k \ne k^\star}
  \exp\!\left(
    -\, \frac{\Big( \Delta_k - \sqrt{\sigma^2_{\mathrm{extrap}, k}}
      - \sqrt{\sigma^2_{\mathrm{extrap}, k^\star}} \Big)_+^2}
    {2 \big( v_k(C) + v_{k^\star}(C) \big)}
  \right),
\]
where $(x)_+ = \max(x, 0)$. As $C \to \infty$ with the design shape (the relative
weighting of $S_{\mathrm{fit}}$ across scales) held fixed, $v_k(C) \to 0$ for every
$k$, while $\sigma^2_{\mathrm{extrap}, k}(S_{\mathrm{fit}})$ does not depend on $C$ at
all.
\end{theorem}

\begin{remark}[This is not the additive form]
\label{rem:not-additive}
An earlier draft of this bound (and \texttt{src/pdt/theory/bound.py}'s already-shipped
\texttt{marginal\_bound\_term}, used throughout P1-07/P1-08's analysis) instead adds
...
counter-example (a fixed bias of $10$ against a gap of $5$ and noise variance $0.01$) %NUMBER-OK: hand-chosen illustrative constants, also in tests/test_bound.py::test_worst_case_form_corrects_a_real_violation_of_the_additive_form -- not a reported result
...
Step~\ref{sec:theorem1-step2}'s delta method is exact rather than a linearization. For
a \emph{nonlinear} $g$, $\hat\theta_k$ is an M-estimator minimizing a (noisy,
misspecified) sum of squares; under Assumption~\ref{asn:family}'s smoothness plus a
local strong-convexity condition on the population objective
$\theta \mapsto \mathbb{E}_{s \sim \pi}[(g(\theta, s) - \mu_k(s))^2]$ near
$\theta_k^{\dagger}$ (i.e. a non-degenerate Hessian there -- without this, the fit is
not even locally identifiable, and Theorem~\ref{thm:identifiability} is precisely about
when it holds), standard M-estimator theory
\needshuman{Concentration of $\hat\theta_k$ around $\theta_k^\dagger$ for a general
nonlinear, misspecified $g$ is asymptotically standard (van der Vaart, 1998, Ch.~5:
consistency and asymptotic normality of $Z$-/$M$-estimators), but making the
\emph{non-asymptotic} sub-Gaussian concentration radius fully explicit -- with
constants tied to the local Hessian's condition number and $g$'s Lipschitz constants,
valid at finite $C$ rather than only as $C \to \infty$ -- for an arbitrary member of
Assumption~\ref{asn:family}'s family is genuine technical work, not a citation lookup.
The six concrete fitters in \texttt{src/pdt/scaling/fitters.py} are well-behaved in
practice (P1-07 cross-checked the delta-method variance against bootstrap variance and
found close agreement), which is evidence the condition holds for them at the sample
sizes DataDecide provides, but is not a substitute for the explicit non-asymptotic
statement.} gives $\hat\theta_k - \theta_k^{\dagger}$ sub-Gaussian with covariance
(governed by) $\Sigma_{\theta_k}(C)$, the sandwich covariance of
Section~\ref{sec:theorem1-step2}.
...
$\mathrm{bias}_k^2 = \sigma^2_{\mathrm{extrap}, k}$, and is (by Steps 1--2)
...
iff the pairwise-ordering condition holds}
...
\label{cor:pairwise}
As $C \to \infty$ (fixed design shape), $\hat{k} \to k^\star$ (selection is eventually
always correct) if and only if, for every $k \ne k^\star$:
\[
  \Delta_k + \mathrm{bias}(D_k) > 0.
\]
...
\begin{proof}
As $C \to \infty$, $v_k(C), v_{k^\star}(C) \to 0$ (Step 2), so $D_k \to D_k^\dagger$
a.s. $\hat{k} = k^\star$ for all $k \ne k^\star$ iff $D_k > 0$ for all $k \ne k^\star$,
which in the limit holds iff $D_k^\dagger > 0$, i.e. $\Delta_k + \mathrm{bias}(D_k) > 0$
(Definition~\ref{def:pairwise-d}). Mechanical.
\end{proof}
...
of the proof from Step~\ref{sec:theorem1-step1}'s nonlinear-$g$ caveat.
