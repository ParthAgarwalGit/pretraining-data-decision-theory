...
(\texttt{bias\_variance\_decomposition} in \texttt{src/pdt/analysis/bootstrap.py}) is
\[
  \widehat{\sigma^2_{\mathrm{extrap}, k}}
  \;=\; \max\!\Big(0,\ \widehat{\mathrm{bias}}_k^2
    - \frac{\hat{v}_k}{B} - \sigma^2_{\mathrm{target}, k} \Big),
\]
where $\widehat{\mathrm{bias}}_k = \overline{\hat\mu}_k^{(1:B)} - \mu_{k,\mathrm{true}}$
is the mean bootstrap prediction of $\mu_k(s^\star)$ across $B$ replicates minus the
measured ground truth, $\hat v_k$ is the across-replicate variance of those same $B$
predictions, and $\sigma^2_{\mathrm{target}, k}$ is the noise in the ground-truth
measurement itself (Assumption~\ref{asn:noise} evaluated at $s^\star$). The subtracted
terms debias the naive $\widehat{\mathrm{bias}}_k^2$, which over-estimates
$\sigma^2_{\mathrm{extrap}, k}$ because $\widehat{\mathrm{bias}}_k$ itself carries
sampling noise from two independent sources (the bootstrap mean's own variance
$\hat v_k / B$, and the ground truth's own measurement noise $\sigma^2_{\mathrm{target}, k}$);
the $\max(0, \cdot)$ clip is necessary because the debiased quantity can go negative in
a finite sample even though the population quantity it estimates cannot.
\end{remark}
