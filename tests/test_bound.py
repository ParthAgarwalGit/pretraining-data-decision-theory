...
def test_plug_in_and_worst_case_forms_both_reject_the_additive_counterexample():
    # The concrete adversarial instance that first exposed the additive-form
    # bug: arm k has a large, effectively-fixed bias (sqrt(sigma2_extrap_k)=10)
    # and tiny variance (0.01); k* is bias-free and noise-free. The true error
    # probability is ~1 (the fixed bias alone dwarfs the gap of 5). The
    # ORIGINAL additive plug-in form claimed ~0.8825079 -- a real violation.
    # PR #17's fix changed `_bound_term` to the gap-reduction form, so the
    # plug-in `marginal_bound_term` now agrees with the worst-case form here:
    # both report 1.0 (vacuous, honest given the bias alone can flip the gap).
    delta_k = 5.0
    sigma2_extrap_k = 10.0**2
    v_k = 0.01

    assert bound.marginal_bound_term(delta_k, sigma2_extrap_k, v_k) == 1.0
    # The original additive value, kept only to document what was fixed.
    additive_value = float(np.exp(-(delta_k**2) / (2 * (sigma2_extrap_k + v_k))))
    assert additive_value == pytest.approx(0.8825079327617985, rel=1e-9)
...
def test_known_noise_v_k_ill_conditioned_full_rank_design_matches_direct_svd():
    # Same design as the third review of PR #17 (cond(J) ~ 2.4e8): known-noise variance must come
    # from the design's SVD, not a pinv of J^T J.
    model, scales, _ys, target = _reviewer_ill_conditioned_case()
    jac = np.array([model.jacobian(s) for s in scales])
    j_target = model.jacobian(target)
    sigma2 = 0.0025
    expected = float(np.sum((np.linalg.pinv(jac).T @ j_target) ** 2 * sigma2))
    assert expected > 1e5
    got = bound.known_noise_v_k(model, scales, lambda s: sigma2, target)
    assert got == pytest.approx(expected, rel=1e-4)
